...
test('rapid Back taps use one transition and deliberate Back remains available afterward', async t => {
  ...
  await h.press('onboardBack'); stage(h, 'intention');
  await h.advance(30);
...
test('Intention Continue cannot also trigger the newly rendered Preview action', async t => {
  const h = await open(t, { profile: profile('intention') });
  await h.press('continueWord'); stage(h, 'preview');
  await h.advance(30);
  h.control('onboardConnectMyMoney').onPress(); await h.flush();
  stage(h, 'preview');
  assert.equal(h.control('onboardConnectMyMoney').disabled, true);
  assert.deepEqual(writes(h), []);
  assert.deepEqual(h.routes, []);
  await h.advance(320);
  await h.press('onboardConnectMyMoney'); stage(h, 'capture');
