 ...
 */
{
  ...
  const manualRows = src.match(
    /\{rows\.map\([\s\S]*?(?=\n\s*\{rows\.length === 0)/,
  )?.[0] ?? '';
  ok(
    'manual reminders use one row target and no nested payment or long-press deletion',
    /onPress=\{\(\) => setSelectedReminderId\(bill\.id\)\}/.test(manualRows) &&
      !/onLongPress|t\('markPaid'\)/.test(manualRows),
  );
  ok(
