...
ok('onboarding choice cards fit the 402×874 default viewport from their declared layout budget',
  defaultCaptureLayoutHeight() === 513 &&
...
const walletOverviewActions = walletOverview.match(/<Pressable[\s\S]*?<\/Pressable>/g) ?? [];
ok('selected tabs and representative interactive boundaries use the control contrast token',
  /borderColor: theme\.controlBorder/.test(billsSegments) &&
    walletOverviewActions.length === 2 &&
    walletOverviewActions.every((action) => /borderColor: theme\.controlBorder/.test(action)) &&
    /onPress=\{onOpenBills\}/.test(walletOverview) &&
    /onPress=\{onOpenCurrency\}/.test(walletOverview) &&
    /onPressRight=\{\(\) => router\.push\('\/cards'\)\}/.test(source('src/app/(tabs)/wallet.tsx')) &&
...
ok('Bills manual reminder rows open one labelled detail target',
  /onPress=\{\(\) => setSelectedReminderId\(bill\.id\)\}/.test(bills) &&
...
ok('Flow exposes the composition graphic as one localized text equivalent',
  /accessibilityRole="image"[\s\S]*?accessibilityLabel=\{compositionA11yLabel\}/.test(flow) &&
    /const compositionA11yLabel = slices[\s\S]*?tf\('compositionPercent'/.test(flow));
ok('Flow trend months expose localized cashflow labels and selected state',
  /accessibilityLabel=\{monthCashflowLabel\(m\)\}[\s\S]*?accessibilityState=\{\{ selected: current \}\}/.test(flow));
ok('Flow keeps an always-visible selected-month sentence and large-text month list',
  /<ThemedText[^>]*accessibilityLiveRegion="polite"[\s\S]*?\{monthCashflowLabel\(selectedTrendMonth\)\}/.test(flow) &&
    /!showTrendValues && \([\s\S]*?trend\.map/.test(flow));
ok('Flow and Stats use their shared accessible screen shells',
  /<ScreenScaffold[\s\S]*?tabbed[\s\S]*?headerMode="inline"/.test(flow) &&
    /<ScreenScaffold[\s\S]*?headerMode="native"/.test(stats));
...
ok('Bills segments use the canonical control and a labelled large-text fallback',
  /<SegmentedControl/.test(billsSegments) &&
    /largeText \? \([\s\S]*?role="tablist"[\s\S]*?: \([\s\S]*?<SegmentedControl/.test(billsSegments) &&
    /accessibilityLabel=\{t\('billsTitle'\)\}/.test(billsSegments) &&
    /accessibilityState=\{\{ selected:/.test(billsSegments));
ok('Bills large-text segment tabs retain platform touch-target floors',
  /minHeight: 44/.test(billsSegments) &&
    /Platform\.OS === 'android'[\s\S]*?minHeight: 48/.test(billsSegments));
...
ok('Wallet uses the typed inline scaffold header and labels both disclosures',
  /const walletHeader: ScreenHeaderProps = \{/.test(wallet) &&
    ...
    /accessibilityLabel=\{sourcesDisclosureLabel\}[\s\S]{0,180}accessibilityState=\{\{ expanded: showAllSources \}\}/.test(wallet) &&
