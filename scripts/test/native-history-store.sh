...
for (const [label, root] of roots) {
  for (const locale of ['en', 'ar']) {
    ...
    if (JSON.stringify(keys) !== JSON.stringify(expectedKeys)) {
      console.error(`native-history-store.sh: ${label} ${locale} keys ${JSON.stringify(keys)} do not match the exact 50-key contract.`);
      process.exit(1);
    }
...
console.log('✓ app and nested resource bundles contain exact distinct EN+AR 50-key tables');
NODE
