...
const focalDue = interactionBills.match(
  /\{focalDue && \(\(\) => \{[\s\S]*?(?=\n\s*\{dues\.length > 1)/,
)?.[0] ?? '';
assert.ok(focalDue.length > 0, 'focal card due was not found');
const focalDetailTarget = focalDue.match(/<Pressable[\s\S]*?<\/Pressable>/)?.[0] ?? '';
assert.match(focalDetailTarget, /accessibilityRole="button"[\s\S]*?accessibilityLabel=/);
assert.match(focalDetailTarget, /openCardDetail\(account, item\.due\.id\)/);
assert.doesNotMatch(focalDetailTarget, /<Button|onPayDue|payCardDue/,
  'the statement detail target must not contain a nested payment action');
const focalPaymentAction = focalDue.slice(focalDue.indexOf('</Pressable>') + '</Pressable>'.length);
assert.match(focalPaymentAction, /<Button label=\{t\('markPaid'\)\}[\s\S]*?onPress=\{\(\) => onPayDue\(/);
assert.doesNotMatch(focalPaymentAction, /payCardDue\(/,
  'the payment CTA must use the existing confirmation, never commit directly');
assert.match(interactionBills, /const onPayDue[\s\S]*?setConfirmation\(\{[\s\S]*?onConfirm:[\s\S]*?payCardDue\(/);
assert.doesNotMatch(focalDue, /t\('seeAll'\)/);
...
const manualRows = interactionBills.match(
  /\{rows\.map\([\s\S]*?(?=\n\s*\{rows\.length === 0)/,
)?.[0] ?? '';
assert.ok(manualRows.length > 0, 'manual reminder rows were not found');
assert.match(manualRows, /onPress=\{\(\) => setSelectedReminderId\(bill\.id\)\}/);
...
const activeAccountRows = interactionWallet.match(
  /group\.accounts\.map\([\s\S]*?(?=\n\s*\{activeSources\.length === 0)/,
)?.[0] ?? '';
assert.ok(activeAccountRows.length > 0, 'active Wallet source rows were not found');
assert.match(activeAccountRows, /accessibilityRole="button"/);
assert.match(activeAccountRows, /onPress=\{\(\) => openAccount\(account\)\}/);
assert.match(activeAccountRows, /onLongPress=\{\(\) => setOptionsFor\(account\)\}/);
assert.match(activeAccountRows, /accessibilityLabel=\{`\$\{account\.name\}[\s\S]*?\$\{figureDescription\}`\}/);
...
assert.match(task4Flow, /composition\(/);
assert.match(task4Flow, /router\.push\(`\/transactions\?category=\$\{s\.categories\.join\(','\)\}`\)/);
assert.match(task4Flow, /from '@\/components\/ui\/data-viz'/);

const task4Stats = read('src/app/stats.tsx');
assert.match(task4Stats, /const statsHeader: ScreenHeaderProps = \{/);
assert.match(task4Stats, /back:[\s\S]*?onPress: \(\) => router\.back\(\)/);
assert.match(task4Stats, /<ScreenScaffold[\s\S]*?headerMode="native"[\s\S]*?header=\{statsHeader\}/);
...
assert.match(task4Flow, /accessibilityRole="image"[\s\S]*?accessibilityLabel=\{compositionA11yLabel\}/);
assert.match(task4Flow, /const compositionA11yLabel = slices[\s\S]*?tf\('compositionPercent'/);
assert.match(task4Flow, /accessibilityLabel=\{monthCashflowLabel\(m\)\}[\s\S]*?accessibilityState=\{\{ selected: current \}\}/);
assert.match(task4Flow, /<ThemedText[^>]*accessibilityLiveRegion="polite"[\s\S]*?\{monthCashflowLabel\(selectedTrendMonth\)\}/);
assert.match(task4Flow, /!showTrendValues && \([\s\S]*?trend\.map/);
...
assert.match(task5Segments, /<SegmentedControl/);
assert.match(task5Segments, /largeText \? \([\s\S]*?role="tablist"[\s\S]*?: \([\s\S]*?<SegmentedControl/);
assert.match(task5Segments, /accessibilityLabel=\{t\('billsTitle'\)\}/);
assert.match(task5Segments, /accessibilityState=\{\{ selected:/);
assert.match(task5Segments, /minHeight: 44/);
assert.match(task5Segments, /Platform\.OS === 'android'[\s\S]*?minHeight: 48/);
for (const label of [
  /`\$\{t\('subscriptionsSeg'\)\} \$\{subscriptionCount\}`/,
  /`\$\{t\('cardsSeg'\)\} \$\{cardCount\}`/,
  /`\$\{t\('utilitiesSeg'\)\} \$\{utilityCount\}`/,
]) assert.match(task5Segments, label);
...
assert.match(task6Wallet, /accessibilityLabel=\{sourcesDisclosureLabel\}[\s\S]{0,180}accessibilityState=\{\{ expanded: showAllSources \}\}/);
...
const headlineAmount = task6Balance.match(
  /<View style=\{\[styles\.overviewAmount[\s\S]*?(?=\n\s*\{activeSourceCount === 0)/,
)?.[0] ?? '';
