...
function harness({ state = 'ready', fail = false } = {}) {
  ...
  return { shadow, calls, store, policy };
}
...
test('cold queued observations never initiate model preparation', async () => {
  const { shadow, calls } = harness({ state: 'not-downloaded' });
  for (let i = 0; i < 5; i += 1) shadow.queueLocalSemanticParserShadow(source, event);
  await shadow.flushLocalSemanticParserShadow();
  assert.equal(calls.warm, 0);
  assert.equal(calls.encode.length, 0);
  assert.equal(shadow.localSemanticShadowSnapshot().modelUnavailable, 5);
});
...
test('a lifecycle reset releases queued windows before starting native inference', async () => {
  const {shadow, calls, policy} = harness();
  shadow.queueLocalSemanticParserShadow(source, event);
  shadow.queueLocalSemanticParserShadow(source, event);
  policy.cancelLocalSemanticBackgroundWork();
  await shadow.flushLocalSemanticParserShadow();
  assert.equal(calls.encode.length, 0);
  assert.equal(shadow.localSemanticShadowSnapshot().queued, 0);
});


test('direct cold observation does not bypass deferred root warmup', async () => {
  const {shadow, calls} = harness({state: 'not-downloaded'});
  await shadow.observeLocalSemanticParserShadow(source, event);
  assert.equal(calls.warm, 0);
  assert.equal(calls.encode.length, 0);
  assert.equal(shadow.localSemanticShadowSnapshot().modelUnavailable, 1);
});


test('shadow admission probe is ready-and-active only and never warms the model', () => {
  for (const state of ['not-downloaded', 'downloading', 'failed', 'ready']) {
    const {shadow, calls, policy} = harness({state});
    assert.equal(shadow.canCollectLocalSemanticShadow(), state === 'ready', state);
    policy.setLocalSemanticAppActive(false);
    assert.equal(shadow.canCollectLocalSemanticShadow(), false, state);
    policy.setLocalSemanticAppActive(true);
    assert.equal(shadow.canCollectLocalSemanticShadow(), state === 'ready', state);
    assert.equal(calls.warm, 0);
    assert.equal(calls.encode.length, 0);
