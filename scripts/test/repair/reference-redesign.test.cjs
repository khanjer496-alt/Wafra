...
test('Activity limits eager row rendering to thirty without dropping the full-ledger link',()=>{
 const h=createHarness({params:{view:'activity'}});h.state.transactions=Array.from({length:100},(_,i)=>({id:'t'+i,title:'Unique merchant '+i,amountFils:100,type:'expense',category:'other',accountId:'enbd',date:'2026-09-06'}));const tree=h.render('flow');assert.equal(walk(tree).filter(n=>n.props?.accessibilityLabel?.includes('Unique merchant')&&n.props.onPress).length,30);button(tree,'View all spending');
});
...
test('Trends merchant rows open a scoped transaction list',()=>{const h=createHarness({params:{view:'trends'}}),tree=h.render('flow');const row=select(tree,n=>n.props?.accessibilityLabel?.startsWith('Talabat,')&&n.props.onPress);row.props.onPress();assert.equal(h.events.at(-1)[1],'/transactions?type=expense&merchant=Talabat')});
