...
test('Bills exposes Upcoming, category views and All while retaining the add-bill sheet',()=>{const h=createHarness(),tree=h.render('bills');const tabs=walk(tree).filter(n=>n.props?.accessibilityRole==='tab');assert.deepEqual(tabs.map(n=>n.props.accessibilityLabel),['Upcoming','Subscriptions','Utilities','Credit cards','All']);button(tree,'All').props.onPress();assert.deepEqual(h.events.at(-1),['state',0,'all']);const scaffold=select(tree,n=>n.type==='Scaffold');scaffold.props.header.actions[0].onPress();assert.deepEqual(h.events.at(-1),['state',7,true])});
