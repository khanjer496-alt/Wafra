...
test('Bills keeps Upcoming / Recurring and the existing add-bill sheet',()=>{const h=createHarness(),tree=h.render('bills');button(tree,'Recurring').props.onPress();assert.deepEqual(h.events.at(-1),['state',0,'recurring']);const scaffold=select(tree,n=>n.type==='Scaffold');scaffold.props.header.actions[0].onPress();assert.deepEqual(h.events.at(-1),['state',7,true])});
