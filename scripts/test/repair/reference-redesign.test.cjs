...
test('Activity limits eager row rendering to thirty without dropping the full-ledger link',()=>{
 const h=createHarness({params:{view:'activity'}});h.state.transactions=Array.from({length:100},(_,i)=>({id:'t'+i,title:'Unique merchant '+i,amountFils:100,type:'expense',category:'other',accountId:'enbd',date:'2026-09-06'}));const tree=h.render('flow');assert.equal(walk(tree).filter(n=>n.props?.testID==='transaction-details-link'&&n.props?.accessibilityLabel?.includes('Unique merchant')&&n.props.onPress).length,30);button(tree,'View all spending');
});
