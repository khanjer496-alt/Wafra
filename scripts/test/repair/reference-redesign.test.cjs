...
test('Spending exposes three views, not a separate Stats button',()=>{const h=createHarness(),tree=h.render('flow');const tabs=walk(tree).filter(n=>n.props?.accessibilityRole==='tab');assert.deepEqual(tabs.map(n=>n.props.accessibilityLabel),['Categories','Compare','Calendar']);button(tree,'Compare').props.onPress();assert.deepEqual(h.events.at(-1),['state',0,'compare'])});
...
test('category filter controls the one category list',()=>{const h=createHarness({states:{1:'unlimited'}}),tree=h.render('flow');const noLimit=h.deps['@/lib/reference-copy'].spendingCopy.en.noLimit;const categories=walk(tree).filter(n=>n.props?.accessibilityLabel?.includes(noLimit)&&n.props.onPress);assert.equal(categories.length,1);assert.ok(categories[0].props.accessibilityLabel.startsWith('Other.'))});
test('category detail passes the selected category into the existing limit sheet',()=>{const h=createHarness({states:{5:'dining'}}),tree=h.render('flow');const sheet=select(tree,n=>n.type==='Sheet');const edit=select(sheet,n=>n.props?.accessibilityLabel===h.deps['@/lib/reference-copy'].spendingCopy.en.manage);edit.props.onPress();assert.deepEqual(h.events.slice(-2),[['state',4,'dining'],['state',5,null]])});
...
test('category history stays anchored while inspecting an older month and can return to latest',()=>{const h=createHarness({period:{mode:'month',key:'2026-08'},states:{5:'dining',6:'2026-09'}}),tree=h.render('flow');const history=select(tree,n=>n.props?.testID==='category-history');const august=select(history,n=>n.props?.testID==='category-history-month-2026-08');const september=select(history,n=>n.props?.testID==='category-history-month-2026-09');assert.equal(august.props.accessibilityState.selected,true);assert.equal(september.props.accessibilityState.selected,false);button(history,'Latest').props.onPress();assert.deepEqual(plain(h.events.at(-1)),['period',{mode:'month',key:'2026-09'}])});
...
test('Trends chart stays anchored while inspecting an older month and can jump back to latest',()=>{const h=createHarness({params:{view:'trends'},period:{mode:'month',key:'2026-08'},states:{8:'2026-09'}}),tree=h.render('flow');const august=select(tree,n=>n.props?.testID==='cashflow-month-2026-08');const september=select(tree,n=>n.props?.testID==='cashflow-month-2026-09');assert.equal(august.props.accessibilityState.selected,true);assert.equal(september.props.accessibilityState.selected,false);assert.deepEqual(plain(august.props.pressRetentionOffset),{top:18,bottom:18,left:8,right:8});button(tree,'Latest').props.onPress();assert.deepEqual(plain(h.events.at(-1)),['period',{mode:'month',key:'2026-09'}])});
...
test('Bills exposes Next 30 days and All, keeps every payment-type view as a filter inside All, and retains the add-bill sheet',()=>{const h=createHarness(),tree=h.render('bills');const tabs=walk(tree).filter(n=>n.props?.accessibilityRole==='tab');assert.deepEqual(tabs.map(n=>n.props.accessibilityLabel),['Next 30 days','All']);button(tree,'All').props.onPress();assert.deepEqual(h.events.at(-1),['state',0,'all']);
 // The subscriptions / utilities / cards views that used to be tabs are still one tap away.
 const all=createHarness({states:{0:'all'}});const allTree=all.render('bills');const filters=walk(allTree).filter(n=>/^bills-filter-/.test(n.props?.testID??''));assert.deepEqual(filters.map(n=>n.props.accessibilityLabel),['Everything','Subscriptions','Utilities & telecom','Card payments']);filters[3].props.onPress();assert.deepEqual(all.events.at(-1),['state',3,'cards']);
 const cardsOnly=createHarness({states:{0:'all',3:'cards'}}).render('bills');assert.match(text(cardsOnly),/Card payments due/);assert.ok(!text(cardsOnly).includes('Netflix'));const scaffold=select(tree,n=>n.type==='BandScaffold');scaffold.props.nav.actions[0].onPress();assert.deepEqual(h.events.at(-1),['state',7,true])});
...
test('Bills records a payment only through the retained confirmation callback',()=>{
 const h=createHarness({states:{4:'dewa'}}),tree=h.render('bills');const sheet=select(tree,n=>n.props?.name==='BillDetailSheet'&&n.props.bill?.bill.id==='dewa');select(sheet,n=>n.props?.accessibilityLabel==='Record as paid').props.onPress();assert.ok(!h.events.some(e=>e[0]==='markBillPaid'));const confirmation=h.events.find(e=>e[0]==='state'&&e[1]===5)?.[2];assert.equal(typeof confirmation.onConfirm,'function');confirmation.onConfirm();assert.ok(h.events.some(e=>e[0]==='markBillPaid'&&e[1]==='dewa'));
});
