...
test('category rows carry exact totals and edit actions',()=>{const h=createHarness(),tree=h.render('flow');const donut=select(tree,n=>n.type==='CategoryDonut');assert.equal(donut.props.centerValue.props.accessibilityLabel,'AED 5,360');assert.match(text(tree),/3,870\.00/);const dining=select(tree,n=>n.props?.accessibilityLabel?.startsWith('Dining.')&&n.props.onPress);dining.props.onPress();assert.deepEqual(h.events.at(-1),['state',5,'dining'])});
...
test('Bills prioritizes the due timeline instead of grouping by payment type',()=>{const h=createHarness(),txt=text(h.render('bills'));const soon=txt.indexOf('Next 7 days'),etisalat=txt.indexOf('Etisalat'),dewa=txt.indexOf('DEWA'),card=txt.indexOf('NBD credit card'),netflix=txt.indexOf('Netflix'),later=txt.indexOf('Later'),spotify=txt.indexOf('Spotify');assert.ok(soon>=0&&soon<etisalat&&etisalat<dewa&&dewa<card&&card<netflix,'near-term obligations stay in chronological order regardless of type');assert.ok(later>netflix&&later<spotify,'later obligations move to the next time section')});
