...
test('Liv partly masked accounts never borrow the unrelated first credit card or fabricate four digits', () => {
  ...
  const sourceAccount = state.accounts.find(a => a.id === liv.accountId);
  assert.ok(sourceAccount);
  assert.equal(sourceAccount.kind, 'bank');
  assert.equal(sourceAccount.bankName, 'Liv');
  assert.equal(sourceAccount.last4, undefined);
  assert.notEqual(liv.accountId, 'unrelated-card');
  ...
  assert.equal(state.accounts.length, 5, 'one issuer and source-mask identity creates one bank account without invented digits');
  assert.deepEqual(apply(state, parse(messages)), state, 'repeated imports reuse the same masked source account');
  const result = core.reconcileTransfers(state.transactions, state.accounts);
...
test('an existing fallback transfer is repaired through exact source identity without changing money or user edits', () => {
  ...
  const sourceAccount = state.accounts.find(a => a.id === state.transactions[0].accountId);
  assert.ok(sourceAccount);
  assert.equal(sourceAccount.kind, 'bank');
  assert.equal(sourceAccount.bankName, 'Liv');
  assert.equal(sourceAccount.last4, undefined);
  assert.notEqual(state.transactions[0].accountId, 'unrelated-card');
  for (const key of ['id', 'amountFils', 'date', 'smsKey']) assert.equal(state.transactions[0][key], old[key]);
  assert.deepEqual(apply(state, parsed), state, 'repeated repair preserves account and row identity');
