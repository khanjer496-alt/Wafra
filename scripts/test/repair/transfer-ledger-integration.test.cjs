...
test('unknown bank transfers on known accounts stay in cash-flow totals until confirmed as own', () => {
  ...
  const sums = transactions => {
    ...
    return [transactions.filter(t => t.type === 'income' && ledger.countsInCashflowTotals(t, undefined, internal)).reduce((s,t) => s+t.amountFils,0),
      transactions.filter(t => t.type === 'expense' && ledger.countsInCashflowTotals(t, undefined, internal)).reduce((s,t) => s+t.amountFils,0)];
  };
  assert.deepEqual(sums(rows), [60000, 12500]);
  assert.equal(core.reconcileTransfers(rows, accounts).pendingIds.size, 2);
  const external = decide(rows, ['out', 'in'], 'external');
  ...
  const own = decide(external, ['out', 'in'], 'own');
...
test('starter budgets use confirmed business income and exclude pending and own transfer credits', () => {
  ...
  assert.equal(onboardingIncomeBasis(decide(pending, pending.map(t => t.id), 'own')), 0);
  assert.equal(onboardingIncomeBasis(decide(pending, pending.map(t => t.id), 'external')), 1000000);
