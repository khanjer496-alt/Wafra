...
test('a generic LLC-style transfer stays reviewable while its known-account cash movement counts', () => {
  ...
  for (const tx of [original, reclassified]) {
    ...
    assert.equal(ledger.isIncome(tx), false, 'commerce income stays conservative while ownership is reviewed');
    assert.equal(ledger.countsInCashflowTotals(tx), true,
      'known-account money in remains visible in cash-flow Net while ownership is reviewed');
  }
});
