...
test('settings and explicit manual entry remain working header actions', () => {
  const h = harness();
  const header = walk(h.tree).find((node) => node.type === 'Scaffold').props.header;
  header.actions[0].onPress(); header.actions[1].onPress();
