...
assert.match(finalSheet, /const bottomClearance = Spacing\.five - 2 \+ \(keyboardHeight > 0 \? 0 : windowBottomInset\)/);
assert.match(finalSheet, /const windowBottomInset = Platform\.OS === 'ios'\n\s+\? Math\.min\(insets\.bottom, initialWindowMetrics\?\.insets\.bottom \?\? insets\.bottom\)\n\s+: insets\.bottom;/);
