...
const notes = `Wafra iOS beta refresh.\n\nPlease test the revised Home and transaction layout, reporting periods, imports, duplicate handling and app responsiveness.\n\nFuture bank SMS: use the Wafra Local Capture Shortcut with a personal Message automation for the bank senders you select. Choose Run Immediately. Wafra keeps the captured message locally and processes the queue when the app opens; setup confirmation is not proof that a real bank alert was captured.\n\nHistory, fixed in this build: tapping Import history previously could return you to Wafra without starting anything, because the app asked Shortcuts for a name that was not the one Apple had installed. It now runs the Shortcut you added. Please add it from the link in the app, tap Import history, and report whether the Shortcut actually opens. Keep the iPhone unlocked while Apple reads Messages; saved batches are kept locally so an interrupted import continues instead of restarting.\n\nStatements: PDF and CSV statements with separate Debit/Credit columns, signed amounts, DD-MMM-YYYY dates or a heading block above the table should now import instead of being refused, and the result reports how many lines could not be read. A password-protected PDF no longer abandons the other files picked with it.\n\nPlease verify captured amounts/accounts and report missed or duplicated alerts through in-app feedback. Real locked-phone delivery, force-quit/reboot behavior and large-inbox speed still need tester verification. Do not erase existing data to troubleshoot this beta.`;
try {
