...
function renderCapabilityMarkdown(rows) {
  ...
  lines.push(
    ...
    '',
    '- Review-only alert vocabularies (not coverage in this matrix) exist for the US, UK, France, Germany, Spain, Italy, Netherlands, India, Qatar, Kuwait, Bahrain, Oman, Egypt and Jordan, and for Canada, Australia, Brazil, Mexico and Singapore. The second group is evidenced only by standard- or community-derived reconstructions with fictional values (`scripts/test/fixtures/public-alert-evidence.js`), not by consented real alerts, so it is excluded from the rows above. See `docs/universal-parser-rollout.md`.',
    '',
