...
> (`2869584d…`) is a retired record in `src/lib/ios-history-setup.ts`; production
> now installs the paged record `5a0da9b5…` generated by
> `scripts/build-ios-paged-history-shortcut.mjs`, whose installed name is
> `Wafra-History-v2-typed-date.signed`. The column-framed successor and the
> run-name repair are described in
