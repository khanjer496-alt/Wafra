...
> **STATUS — SUPERSEDED FOR NEW BUILDS.** The per-message graph below
> (`2869584d…`) is a retired record in `src/lib/ios-history-setup.ts`. Since
> 19 September 2026 every iOS profile installs the typed-date paged graph
> **Wafra History v4** (`scripts/build-ios-paged-history-shortcut.mjs --rows`,
> Apple-signed release asset `ios-history-v4-20260919/Wafra-History-v4.signed.shortcut`,
> installed name `Wafra-History-v4.signed`). Its rationale and verification are in
> [`test-evidence/ios-history-v4-typed-dates-2026-09-19.md`](./test-evidence/ios-history-v4-typed-dates-2026-09-19.md).
> The earlier v2 record (`bc30c7ae…`, `Wafra-History-v2-typed-date.signed`) and
> the column-framed v3 candidate
> ([`performance/ios-history-column-framing-2026-09-13.md`](./performance/ios-history-column-framing-2026-09-13.md))
> formatted dates in Shortcuts, which iOS 26 returned as display strings.
> The 3,000-Message coverage boundary and the 23-minute measurement below apply
> only to the retired per-message graph.
