...
For an internal full-inbox export APK, dispatch the existing `build-apk.yml` workflow against the reviewed source with `architecture=arm64`, `corpus=true`, and `bundle=false`. Bank-notification access is available in normal builds after the user grants Android Notification access; it no longer needs a separate build input. The corpus workflow also enables the separate redacted parser-samples screen. Verify the output version is newer than the installed APK and the signing certificate matches before offering it as an update. Do not publish this export APK to the public download page or Play Store.
