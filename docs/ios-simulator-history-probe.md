...
| 2 | Shuts the device down, writes the corpus into `Library/SMS/sms.db` with `scripts/ios-sim/seed-sms-db.py`, boots, opens Messages. | The simulator's Messages has been observed to keep conversations in memory and ignore store writes; the stage fails when the seeded conversations are not listed. |
...
| 3 | Builds the simulator app (fingerprint-cached, shared with `native-sim.yml`), installs and launches it with the v6 history record configured. | Native build failures. |
| 4 | Downloads the signed shortcut from its release asset, serves it on loopback, tries the `shortcuts://import-shortcut` URL, then the Safari-download route the app's own help text describes. | Apple may refuse either route on a simulator; both leave screenshots. |
