...
| 3 | Builds the simulator app (fingerprint-cached under the probe's own key), installs and launches it with the v6 history record configured, and records the first screen plus the app's log lines. | Run 1: a build made with `CODE_SIGNING_ALLOWED=NO` opens on "Your ledger could not be opened" (SecureStore cannot reach the keychain). Run 3: the same source built without that flag opens on the welcome screen, and the `wafra://ios-setup?section=history` link reaches the Bank messages setup. `native-sim.yml` still disables signing. |
| 4 | Tries `shortcuts://import-shortcut` with the public https release-asset URL, then the Safari-download route the app's own help text describes, then an iCloud share link of an older verified record; "installed" means `open-shortcut` by name opens the editor rather than a "Could not find the shortcut" sheet. Stage 5 runs whichever name was installed. | Runs 1 and 3: the import URL scheme refuses both a loopback http URL and the public https asset URL ("The shortcut URL provided was invalid"), so it is not a route on the simulator. The iCloud fallback proves only that Shortcuts can take a Wafra history graph, not that the release asset installs. |
