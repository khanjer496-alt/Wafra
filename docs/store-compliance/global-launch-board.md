# Wafra global launch board

_Status snapshot: 10 September 2026._

| Area | Status | Evidence / next gate |
| --- | --- | --- |
| Global ledger currency engine (0/2/3 ISO minor units) | DONE | Explicit `ledgerMoney` is persisted and used by formatting/import guards. |
| First manual monetary action requires currency | DONE | Transaction/account/budget/bill/goal paths cannot create money on a fresh ledger without a selected currency. |
| Worldwide positioning | DONE | English default listing; automatic bank-alert claims remain limited to verified packs/formats. |
| Android SMS disclosure architecture | DONE | `READ_SMS` and `RECEIVE_SMS` have separate purposes; final artifact video still required. |
| Store metadata/pricing validators | DONE | Global English launch metadata and storefront-native price strategy are machine checked. |
