...
export default function MerchantRoute() {
  ...
  return <MerchantScreen key={merchant} merchant={merchant} />;
}

function MerchantScreen({ merchant }: { merchant: string }) {
  ...
  const [view, setView] = useState<'spending' | 'all'>('spending');
  ...
  const matches = view === 'spending' ? summary.spending : summary.activity;
  ...
  const renderRow = useCallback(({ item }: { item: Transaction }) => <View style={[styles.transaction, { borderColor: theme.cardBorder }]}>
    ...
    {!countsInTotals(item, live, internal) && <ThemedText type="meta" themeColor="textSecondary">{w.excludedRow}</ThemedText>}
  </View>, [theme.cardBorder, accountById, internal, live, openEntry, w.excludedRow]);

  return <>
    <ScreenScaffold scroll={false} virtualized testID="merchant-detail"
      header={{ title: w.activity, back: { label: w.back, onPress: () => router.canGoBack() ? router.back() : router.replace('/merchants') } }}>
      ...
      <FlatList data={data} renderItem={renderRow} keyExtractor={transactionKey}
        ...
        ListHeaderComponent={<View style={styles.header}>
          <View style={styles.identity}><MerchantAvatar title={merchant} category={summary.activity[0]?.category ?? 'other'} size={48} />
            <ThemedText type="heading" style={styles.name}>{merchant || w.pick}</ThemedText></View>
          ...
          <View style={styles.hero} testID="merchant-total-spent">
            <ThemedText type="small" themeColor="textSecondary">{w.total}</ThemedText>
            <Money fils={summary.totalFils} type="display" />
          </View>
          <View style={[styles.facts, { borderColor: theme.cardBorder }, large && styles.stack]}>
            <View style={styles.fact}><ThemedText type="meta" themeColor="textSecondary">{w.purchases}</ThemedText>
              <ThemedText type="heading" tabular testID="merchant-purchase-count">{summary.spending.length}</ThemedText></View>
            {summary.averageFils !== null && <View style={styles.fact}>
              <ThemedText type="meta" themeColor="textSecondary">{w.average}{summary.averageApproximate ? ' ≈' : ''}</ThemedText>
              <Money fils={summary.averageFils} type="smallBold" /></View>}
          </View>
          {summary.received.length > 0 && <View style={styles.received} testID="merchant-money-received">
          ...
          <ThemedText type="meta" themeColor="textSecondary">{w.exclusions}</ThemedText>
          <SegmentedControl value={view} onChange={setView} label={w.activity} segments={[
            { value: 'spending', label: w.spending }, { value: 'all', label: w.allActivity },
          ]} />
        ...
        ListEmptyComponent={<View style={styles.empty}><ThemedText type="smallBold">{view === 'spending' ? w.empty : w.emptyActivity}</ThemedText>
          <Button label={merchant ? w.allTime : w.pick} variant="outline" onPress={() => merchant ? setPeriod({ mode: 'all' }) : router.replace('/merchants')} />
        </View>}
        ListFooterComponent={<View style={styles.footer}>
          {merchant && <View testID="merchant-view-all-transactions"><Button label={w.viewAllTransactions}
            variant="outline" icon="arrow-up-right" wrapLabel
            onPress={() => router.push(`/transactions?type=all&merchant=${encodeURIComponent(merchant)}`)} /></View>}
          {summary.hasConvertedAmounts && <ThemedText type="meta" themeColor="textSecondary">{w.converted}</ThemedText>}
          <ThemedText type="meta" themeColor="textSecondary">{w.identity} {w.sharedPeriod}</ThemedText>
        </View>} />}
    </ScreenScaffold>
