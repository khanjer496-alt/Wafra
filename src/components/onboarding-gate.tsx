...
type Step =
  ...
  | 'capture'
  | 'complete';
const JOURNEY_STEPS: readonly Step[] = ['focus', 'tracking', 'preview'];
...
export function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  const startScan = async () => {
    ...
    try {
      ...
      saveJourney('complete');
      // Save the configured-capture checkpoint before the reveal. The final
      // Open Wafra action still owns the durable onboarded=true transition.
      await ensureDurable();
      setCompletionOutcome('automatic');
      setStep('complete');
    } catch {
...
const styles = StyleSheet.create({
  ...
  welcomeTop: { paddingTop: 14, gap: 10 },
  ...
  brandName: { color: night.textSecondary, fontFamily: Fonts.sansSemi, fontSize: 15, letterSpacing: 0.2 },
  eyebrow: {
    paddingTop: 12,
    color: night.primary,
  ...
  sub: { fontFamily: Fonts.sans, fontSize: 15, lineHeight: 22, color: night.textSecondary, maxWidth: 390 },
