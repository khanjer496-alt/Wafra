...
function ReminderCard({ title, when, palette, trailing, testID }: {
  title: string;
  when: string;
  palette: BandPalette;
...
export function RemindersStep({ dailySummary, onDailySummary, onAllow, onNotNow, onBack, onClose, busy }: {
  ...
  const included = <View style={styles.included} accessible accessibilityLabel={words.remindIncluded}>
    <Icon name="check" size={16} color={band.onFill} strokeWidth={2.2} />
    <ThemedText type="micro" style={{ color: band.onFill }}>{words.remindIncluded}</ThemedText>
  </View>;
  return <EStepFrame palette={band} step={4} onBack={onBack} onClose={onClose} backDisabled={busy} testID="onboarding-reminders"
    ...
    <View style={styles.cards}>
      <ReminderCard palette={band} title={words.remindBillsTitle} when={words.remindBillsWhen} trailing={included}
        testID="onboarding-remind-bills" />
      <ReminderCard palette={band} title={words.remindCardsTitle} when={words.remindCardsWhen} trailing={included}
      ...
      <ReminderCard palette={band} title={words.remindDailyTitle} when={words.remindDailyWhen} testID="onboarding-remind-daily"
        trailing={<Switch value={dailySummary} onValueChange={onDailySummary} disabled={busy}
          accessibilityLabel={`${words.remindDailyTitle}. ${words.remindDailyWhen}`}
          trackColor={{ true: band.accent, false: band.bandMark }} ios_backgroundColor={band.bandMark}
          testID="onboarding-remind-daily-switch" />} />
    </View>
  </EStepFrame>;
}
...
const styles = StyleSheet.create({
  ...
  included: { flexDirection: 'row', alignItems: 'center', gap: 4, maxWidth: 120 },
});
