...
export function PaymentAgenda({ items, accounts = [], includePaid, group: selectedGroup, onOpen }: {
  ...
  const groups = selectedGroup
    ...
    : groupPaymentKinds(items, includePaid);
    ...
    : key === 'utilities'
      ...
      : key === 'cards'
        ...
        : key === 'loans'
          ? w.emptyLoans
          : w.emptyOther;
  const visibleCount = groups.reduce((sum, itemGroup) =>
    sum + itemGroup.sections.reduce((sectionSum, section) => sectionSum + section.items.length, 0), 0);
