import { localSemanticBackgroundCancellation, onLocalSemanticBackgroundCancelled } from '@/lib/local-semantic-background-policy';
...
export async function observeLocalSemanticParserShadow(
  ...
  if (localSemanticBackgroundCancellation()()) return;
...
let draining = false;
onLocalSemanticBackgroundCancelled(() => { queue.length = 0; state.queued = 0; });

/** Avoid constructing optional parser inputs when shadow inference cannot run. */
export const canCollectLocalSemanticShadow = (): boolean =>
  !localSemanticBackgroundCancellation()() && localSemanticRuntimeStatus().state === 'ready';
...
async function drainShadowQueue(): Promise<void> {
  ...
  try {
    while (queue.length > 0) {
      ...
      if (cancelled()) break;
      if (localSemanticRuntimeStatus().state !== 'ready') {
        // Optional observations must never initiate a cold model download or
        // session. Root warmup and explicit interactive requests own preparation.
      ...
      if (!next) break;
      await scoreShadowWindow(next.window, next.family, cancelled);
      state.queued = queue.length;
      persistIfDue();
      await yieldTurn();
    }
    ...
    draining = false;
    persistIfDue(true);
    // A reset may have cancelled the old drain while new active work arrived.
    if (queue.length && !localSemanticBackgroundCancellation()()) void drainShadowQueue();
  }
}
...
async function scoreShadowWindow(
  ...
  cancelled = localSemanticBackgroundCancellation(),
): Promise<void> {
  try {
    if (cancelled()) return;
    const encoder = await getLocalSemanticEncoder();
    if (cancelled()) return;
    const prediction = hybridPrediction(await encoder.encode(semanticText, { priority: 'background', cancelled }));
    if (cancelled()) return;
    ...
    if (!cancelled()) state.modelUnavailable += 1;
  }
}
