...
async function loadPersisted(): Promise<LoadedState | null> {
  ...
  if (!Array.isArray(parsed.transactions)) {
    ...
    if (count > 0) {
      ...
      for (const [, v] of pairs) {
        if (v) {
          txs.push(...(JSON.parse(v) as Transaction[]));
          chunkBodies.push(v);
        }
      }
    }
  ...
  return { state: parsed, chunkBodies };
}
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  /**
   * Persist. Two guards, because this runs on EVERY dispatch.
   *
  ...
  const persist = useCallback((snapshot: AppState) => {
    ...
    // Only chunks whose contents actually changed are rewritten.
    const changed = chunks ? chunks.filter(([, body], i) => prevChunks.current[i] !== body) : [];
