 ...
 * transfers, card purchases and settlements, utility payments, ATM cash,
 * refunds and bank fees without depending on one bank sentence template.
 * Existing UAE/Saudi grammar results stay authoritative for money and specific
 ...
 *
 * 30: corpus identity repairs. FastPay's "account ending with" retains the
 * stated account tail, and a hyphenated ON-PUTTHA surname stays whole. Reread
 * retained history so parser-owned rows can recover their title and uniquely
 * resolved account through the existing guarded import healing path. Import
 * also retains source instrument evidence for duplicate matching and stops
 * unattributed snapshots from overwriting fallback accounts.
 *
 * 31: proactive parser safeguards. Biller-reference footers cannot rename
 * purchases or invent utility categories, user-pinned categories survive,
 * and bill identities require numeric evidence. Statement deadlines and
 * abbreviated total/minimum labels retain their separate meanings. Malformed
 * local-money tokens cannot be truncated into amounts or restored by semantic
 * fallback; unreadable balance tokens remain unknown.
 *
 * 32: statements require a readable total. A minimum-only reminder is not
 * evidence of the full debt and cannot create a settleable obligation. This
 * prevents new incorrect dues; persisted dues have no source provenance and
 * cannot safely be identified or deleted by comparing total and minimum.
 *
 ...
 *
 * 44: semantic edge-case repair. Proven moves from one owned account into
 * another keep an own-account title instead of the misleading Card payment;
 * explicit paid/settled invoices are retained as business income even when no
 * payer name is present; and an incoming credit explicitly reversed FROM the
 * user's account is an outgoing credit reversal rather than a refund/income.
 * These are source-proven meaning corrections only: amount, date and account
 * extraction are unchanged, and no remote inference runs in the parser.
 *
 * 45: user-provided ADIB history repair. The bank's subject-first credit-card
 * repayment receipt ("your payment ... for card ... has been credited") is a
 * card settlement instead of fresh spending; cheque deposits explicitly sent
 * for clearing remain non-posting until the bank later says they cleared; and
 * the cleared-cheque receipt's punctuated "AED." label is accepted as local
 * money. ADIB profit credits also keep a bank-profit title instead of becoming
 * ownership-unknown incoming transfers that disappear from exact Income/Net.
 * Re-read retained Android history so old repayment expenses, premature cheque
 * credits and hidden profit income can heal from their original source evidence.
 *
 * 46: expanded ADIB history dataset repair. A failed card attempt worded
 * "CVV ... was incorrect" now stays non-posting just like "invalid CVV"; and
 * the account-side settlement order "AED X payment for your Covered Card was
 * debited from your account" is retained as the debit leg of a card payment,
 * not fresh spending. Re-read retained Android history so either legacy error
 * can heal from the original SMS without duplicating correctly imported rows.
 */
...
export interface ParsedSms {
   ...
   * cardStatement — credit card summary; date is null if its deadline is unknown;
...
/**
 * An instruction to approve a transaction is an authorisation request, not a
 * posting. Keep this word order deliberately imperative: "approve this card
 * purchase" has not moved money, while "your card purchase was approved" may
 * be the bank's completed authorisation alert and must continue through the
 * ordinary posted/non-posted evidence checks.
 */
const TRANSACTION_APPROVAL_REQUEST_RE =
  /(?:^|[.!?]\s*)\s*(?:please\s+)?(?:tap\s+(?:here\s+)?to\s+)?(?:approve|confirm|authoris(?:e|z)e)\s+(?:this\s+|the\s+|your\s+)?(?:card\s+)?(?:purchase|payment|transaction|transfer)\b|\b(?:open|use)\s+(?:the\s+|your\s+)?(?:bank|mobile)?\s*app\s+to\s+(?:approve|confirm|authoris(?:e|z)e)\s+(?:this\s+|the\s+|your\s+)?(?:card\s+)?(?:purchase|payment|transaction|transfer)\b|^\s*(?:(?:يرجي|الرجاء)\s+(?:الموافقه|التاكيد)\s+علي|قم\s+ب?(?:الموافقه|تاكيد))\s+(?:عمليه\s+)?(?:شراء|دفع|تحويل)(?![\u0621-\u064A])/i;

/**
 * Explicit challenges may glue the code to OTP. Keep the request verb and
 * terminal code/continuation together: an ordinary "never share your OTP"
 * footer has no quoted code, and "USE OTP123456 CAFE" is still a seller.
 * Read the body because a trailing "requires OTP123456" can be swallowed by
 * the merchant extractor; blanking that mistaken merchant would hide the gate.
 */
const GLUED_OTP_REQUEST_RE = new RegExp(
  String.raw`\b(?:requires?|enter|use)\s+(?:your\s+)?otp${OTP_CODE_DIGITS}(?=\s*(?:[.!?;]|$|\b(?:to|for|within|before)\b))`,
  'i',
);
...
/**
 * The opposite side of a refund: an earlier INCOMING credit was taken back.
 *
 * A generic "transaction ... reversed" is still a refund below. This pattern
 * is deliberately narrower: it requires the message to name an incoming-money
 * concept, say that it was reversed, AND say the money was reversed FROM the
 * user's account/card. That is enough to establish an outgoing movement without
 * turning ordinary purchase reversals into expenses.
 */
const INCOMING_CREDIT_REVERSAL_RE =
  /\b(?:salary|payroll|wages?|wps|incoming\s+(?:payment|transfer)|inward\s+remittance|deposit(?:ed)?|credit(?:ed)?)\b(?:[^.\n]|\.\d){0,120}?\b(?:was\s+|has\s+been\s+)?reversed\b(?:[^.\n]|\.\d){0,72}?\bfrom\s+(?:your|the)\s+(?:account|a\/?c|card)\b/i;
/**
...
const PROMO_SENTENCE_RE = new RegExp(
  `${PROMO_SENTENCE_CHAR}*(?:` +
    ...
    `|\\b(?:pay|manage)\\b${PROMO_SENTENCE_CHAR}{0,48}?\\b(?:utility|electricity|water|telecom|internet)\\s+bills?\\b${PROMO_SENTENCE_CHAR}{0,32}?\\b(?:app|online)\\b` +
...
const OFFER_RE =
  /^\s*(?:(?:offer|promo(?:tion)?|deal)\s*[:!-]|عرض\b)|\bpurchases?\s+(?:above|over|worth|starting\s+(?:at|from))\s+(?:aed|dhs|sar)\b|\bat\s+any\s+(?:store|shop|merchant|outlet|retailer|branch)\b|\bon\s+purchases\s+(?:above|over)\b|\boff\s+your\s+(?:next|first)\b|\b(?:aed|dhs|sar)\s*[\d,]+(?:\.\d{1,2})?\s+off\b|\bup\s+to\s+(?:aed|dhs|sar)\s*[\d,]+(?:\.\d{1,2})?\s+(?:off|cashback|back)\b|\b(?:for|on|per)\s+(?:every|each)\s+(?:aed|dhs|sar)\s*[\d,]+(?:\.\d{1,2})?\b|\b(?:on|for)\s+(?:aed|dhs|sar)\s*[\d,]+(?:\.\d{1,2})?\s+spent\b|\bup\s+to\s+\d{1,3}\s*x\b|\beach\s+of\s+[\d,]+(?:\.\d{1,2})?\s*(?:aed|dhs|sar)\b/i;
// JavaScript's `\b` is ASCII-only, so Arabic word boundaries have to be
...
const FUTURE_CLAUSE_RE =
  /\b(?:will|shall|would|going\s+to)\s+(?:not\s+)?(?:be\s+)?(?:auto[\s-]?)?(?:debited|deducted|credited|deposited|received|remitted|refunded|reversed|charged|applied|applicable|paid|posted|processed|collected|taken|transferred|withdrawn|deposit|receive|remit|refund|reverse|deduct|debit|charge|credit|apply|applies)\b(?:[^.\n]|\.\d)*|\bscheduled\s+(?:for|on|to\s+be)\b(?:[^.\n]|\.\d)*|سيتم\s+(?:خصم|اضافه|ايداع|تحويل|دفع|سداد)(?:[^.\n]|\.\d)*/gi;
/**
...
/**
 * Evidence that a MONEY MOVEMENT already happened, deliberately excluding an
 * available-balance/limit snapshot. A snapshot can accompany a forecast just
 * as easily as a posting, so it must never rescue a future salary, payout,
 * refund, fee or card payment after that future clause is removed.
 *
 * SETTLED_TENSE_RE remains broader for its existing statement/offer guards;
 * this narrower contract is only for deciding whether a separate future
 * clause is harmless footer text beside a real transaction.
 */
...
const REQUEST_RECEIVED_RE =
  /\b(?:payment|transfer|purchase)\s+request\b(?:[^.]|\.\d){0,64}\b(?:received|submitted|created|awaiting|pending)\b|\b(?:received|submitted|created)\b(?:[^.]|\.\d){0,48}\b(?:payment|transfer|purchase)\s+request\b|تم\s+استلام\s+طلب\s+(?:سداد|دفع|تحويل)/iu;
const MANDATE_LIFECYCLE_RE =
  /\b(?:direct debit|auto[ -]?pay|standing order|mandate)\b(?:[^.]|\.\d){0,96}\b(?:will\s+(?:start|begin|commence|activate)|has\s+been\s+(?:created|registered|set\s*up|activated)|is\s+(?:now\s+)?active)\b|\b(?:created|registered|set\s*up|activated)\b(?:[^.]|\.\d){0,64}\b(?:direct debit|auto[ -]?pay|standing order|mandate)\b|تم\s+(?:تسجيل|انشاء|تفعيل)[^\n]{0,64}(?:امر\s+)?(?:خصم\s+(?:تلقايي|مباشر)|تعليمات\s+مستديمه)/iu;
const CONDITIONAL_PAYOUT_RE =
  /\b(?:merchant|business|seller|vendor|restaurant|platform|marketplace|aggregator|acquirer|sales?|delivery|courier|rider|driver)\b(?:[^.]|\.\d){0,160}\b(?:when|once|if)\s+(?:you\s+)?(?:are\s+)?(?:eligible|qualif(?:y|ied))\b|\b(?:merchant|business|seller|vendor|restaurant|platform|marketplace|aggregator|acquirer|sales?|delivery|courier|rider|driver)\b(?:[^.]|\.\d){0,160}\b(?:after\s+(?:(?:your\s+)?registration|you\s+(?:register|sign\s+up|open\s+(?:an?\s+)?account)|opening\s+(?:an?\s+)?account)|upon\s+(?:registration|sign[ -]?up|account\s+opening))\b/iu;
const CONDITIONAL_MOVEMENT_RE =
  /\b(?:salary|payroll|wages|wps|monthly\s+pay|remuneration|emoluments?|refund|payout|settlement|proceeds|earnings|disbursement)\b(?:[^.]|\.\d){0,144}\b(?:if|when|once|upon)\s+(?:it\s+is\s+|you\s+are\s+)?(?:approved|authori[sz]ed|verified)\b|\b(?:salary|payroll|wages|wps|monthly\s+pay|remuneration|emoluments?|refund|payout|settlement|proceeds|earnings|disbursement)\b(?:[^.]|\.\d){0,144}\bafter\s+(?:completing\s+)?(?:onboarding|registration|verification|account\s+opening)\b/iu;
...
function ensureCurrencyPatterns(): void {
  ...
  CARD_PAYMENT_RE = new RegExp(
    `payment\\s+(?:of\\s+(?:${CUR})\\s*[\\d,.]+\\s+)?(?:is\\s+|was\\s+|has\\s+been\\s+)?(?:received|credited|processed)\\s+(?:towards?|to|on|for)\\s+(?:your\\s+)?${CARD_GAP}` +
      ...
      // ADIB receipt order: "your payment of AED 100.00 on 7/29/2020 for card
      // ending with **0121 has been credited". The destination card appears
      // BEFORE the credit verb, so the receipt-first alternatives above can
      // never reach it. Keep the optional gap to a date only; this must not
      // cross a bill/payee clause and turn a utility payment into settlement.
      `|(?:your\\s+)?payment\\s+of\\s+(?:${CUR})\\s*[\\d,.]+(?:\\s+on\\s+\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4})?\\s+for\\s+(?:your\\s+)?${CARD_GAP}\\s*(?:no\\.?|number|ending(?:\\s+(?:in|with))?)?\\s*[\\dXx*•-]{0,20}\\s+(?:has\\s+been|was|is)\\s+credited\\b` +
      ...
      // ADIB account-side subject order: "AED 42.10 payment for your Covered
      // Card was debited from your account". The card/settlement subject comes
      // before the debit verb, so the existing "debited ... towards payment"
      // alternative cannot reach it. Keep the destination card immediately
      // after payment-for and require the explicit debit-from-your-account
      // clause so an ordinary merchant "payment for X" cannot become a transfer.
      `|(?:${CUR})\\s*[\\d,.]+\\s+payment\\s+for\\s+(?:your\\s+)?${CARD_GAP}\\s+(?:was|has\\s+been|is)\\s+(?:debited|deducted)\\s+from\\s+your\\s+(?:account|a\\/?c)\\b` +
...
/** Keep bad balance tokens unknown; never truncate a transaction's own money. */
function sanitizeLocalMoney(raw: string): string | null {
  ensureCurrencyPatterns();
  const invalid = malformedLocalMoneyTokens(raw, getActiveMarket().currency.aliases.map(foldOrthography));
  if (!invalid.length) return raw;
  for (const token of invalid) {
    const prefix = raw.slice(Math.max(0, token.contextStart - 56), token.contextStart);
    // The existing parking-confirmation corpus quotes a settled two-decimal
    // Fee and an ancillary three-decimal VAT calculation. Do not turn that
    // tax detail into the transaction or discard the separately stated fee.
    const parkingTax = /\bVAT[-:\s]*$/i.test(prefix) &&
      /\bPlateNo[-:]\s*\d+/i.test(raw) && /\bTicketNo[-:]\s*\d+/i.test(raw) &&
      /\bFee[-:]\s*(?:AED|DHS)\s*\d+(?:\.\d{1,2})?(?![\d.,])/i.test(raw);
    if (parkingTax) continue;
    // A statement total is an obligation, even when its label also contains
    // "balance". Dropping it must not make the minimum look like the total.
    if (!BALANCE_PREFIX_RE.test(prefix) || TOTAL_DUE_LABEL_RE.test(prefix) ||
        AR_TOTAL_DUE_LABEL_RE.test(prefix)) return null;
  }
  // Preserve character positions used to slice merchant names out of clean.
  let sanitized = raw;
  for (const token of invalid.slice().reverse()) {
    sanitized = sanitized.slice(0, token.start) + ' '.repeat(token.end - token.start) +
      sanitized.slice(token.end);
  }
  return sanitized;
}

/** Semantic fallback must honor the legacy parser's whole-token money refusal. */
export function hasMalformedBankAmount(message: string): boolean {
  return sanitizeLocalMoney(normalizeArabic(message)) === null;
}
...
function extractBillIdentity(raw: string): string | null {
  ...
  for (const match of matches) {
    ...
    // "customer service" and "account ending" are prose, not identifiers.
    // Keep scanning so a preceding support sentence cannot hide a later ID.
...
const MERCHANT_STOP =
  ...
  // ON-PUTTHA in the real corpus is a surname, not an "on <date>" clause.
  // Keep the existing prose boundary everywhere except inside a hyphenated word.
...
const ATM_RE = /\bcash\s+withdrawal\b[\s\S]{0,32}(?:\b(?:aed|sar|dhs?)\b|\d)|\bcash\s+advance\b[\s\S]{0,96}\bcredit\s+card\b[\s\S]{0,48}\batm\b|\bwithdraw(?:n|al)\b|\batm\b[^.\n]{0,40}\b(?:cash|withdraw)\w*\b|\b(?:cash|withdraw)\w*\b[^.\n]{0,40}\batm\b|(?:تم\s+)?سحب\s+نقدي[\s\S]{0,32}(?:\b(?:aed|sar)\b|\d)|سحب[\s\S]{0,32}(?:الصراف الالي|صراف الي|جهاز الصراف)/i;
const FEE_RE =
...
const CATEGORY_KEYWORDS: [RegExp, CategoryId][] = [
  ...
  [/hyper\s?ramez|lavender al madina|mark\s*(?:and|&)\s*save|fresh good day|almed retail|\btops-patong\b|dubai refreshment|falcon ?pack/i, 'groceries'],
...
const PAID_INVOICE_RE =
  /\binvoice(?:\s+(?:(?:no\.?|number|#)\s*)?[a-z0-9-]+)?\s+(?:was\s+|has\s+been\s+)?(?:paid|settled)\b/i;
...
function businessIncomeTitle(raw: string): string | null {
  ...
  if (PAID_INVOICE_RE.test(raw)) return 'Invoice payment';
...
/**
 * Shared strict category compatibility. Other is valid in either direction,
 * while a merchant rule retains the direction in which the user chose it.
 */
export function overrideFitsDirection(category: CategoryId, type: TransactionType): boolean {
  return categorySupportsType(category, type);
}
...
function categoryOf(
  ...
  if (overrides && merchant) {
    const hit = readMerchantCategoryOverride(overrides, merchant, type);
  ...
  // Specific businesses beat a local pack's broad brand stem: Lulu Exchange
  // is not a supermarket, Danube Home is not Danube's grocery operation, and
  // an stc pay wallet movement is not evidence of a phone-service bill.
  if (/\b(?:al\s*ansari|al\s*fardan|lulu|uae|sharaf|index|orient|wall\s*street|al\s*rostamani|gcc|joyalukkas)\s*exchange\b|\bexchange\s*(?:centre|center|house)\b|\bmoney\s*exchange\b/i.test(merchantText)) {
    return { id: 'other', deliberate: true };
  }
  if (/\bstc\s*pay\b/i.test(merchantText)) return { id: 'other', deliberate: true };
  if (/\bdanube\s+home\b/i.test(merchantText)) return { id: 'shopping', deliberate: true };
  // The naming layer already refuses service-brand collisions with explicit
  // local trade activity. Category lookup must honor that same evidence too.
  if (LOCAL_TRADE_ACTIVITY_RE.test(merchantText)) {
    for (const [re, name] of SERVICE_NAMES) {
      if (AMBIGUOUS_SERVICES.has(name)) text = text.replace(re, ' ');
    }
  }
  ...
  const marketKeywords = marketId === null ? globalCategoryKeywords() :
    marketId === undefined ? getActiveMarket().keywords : keywordsForMarket(marketId);
 ...
 * Pass null for global vocabulary only; undefined retains the legacy active
 * market default, while an explicit market adds that country refinement.
 *
 ...
 *
 * A minimum-only summary is a real corpus shape, but the CardDue model has
 * no unknown-total state. Refuse it until a total is stated; promoting a
...
// Direct deadline fields also omit "is" ("Due Date 23/07/26") or use
// "due on". Their date must beat any preceding statement-generation date.
const DUE_DIRECT_PREFIX = String.raw`\bdue\b\s*(?:date\b\s*)?(?:(?:is|on|by|before)\b\s*|:\s*)?`;
const DUE_DIRECT_NUMERIC_RE = new RegExp(
  DUE_DIRECT_PREFIX + String.raw`(\d{1,2})[/.-](\d{1,2})[/.-](\d{2,4})(?!\d)`, 'i');
const DUE_DIRECT_DAY_NAMED_RE = new RegExp(
  DUE_DIRECT_PREFIX + String.raw`(\d{1,2})[-\s]?([A-Za-z]{3,9})[-\s]?(\d{2,4})(?!\d)`, 'i');
const DUE_DIRECT_NAMED_RE = new RegExp(
  DUE_DIRECT_PREFIX + String.raw`([A-Za-z]{3,9})\.?\s+(\d{1,2}),?\s+(\d{4})(?!\d)`, 'i');
...
function extractDueDate(raw: string): string | null {
  ...
  const direct = raw.match(DUE_DIRECT_NUMERIC_RE);
  if (direct) return numericDate(direct[1], direct[2], direct[3]);
  const directDayNamed = raw.match(DUE_DIRECT_DAY_NAMED_RE);
  if (directDayNamed) return namedDate(directDayNamed[2], directDayNamed[1], directDayNamed[3]);
  const directNamed = raw.match(DUE_DIRECT_NAMED_RE);
  if (directNamed) return namedDate(directNamed[1], directNamed[2], directNamed[3]);
...
/**
 * Day-of-month fallback for statements that omit the year ("due on 25 Aug"
 * or "due 25/08"). The import pipeline only needs this stable day to roll the
 * reminder forward; inventing a year here would be less accurate than keeping
 * the full date unknown.
 */
function extractDueDay(raw: string): number | null {
  const full = extractDueDate(raw);
  if (full) return Number(full.slice(8));
  // A malformed full date is not a partial date. Otherwise regex backtracking
  // can reduce "30/02/2027" to a fabricated day/month fragment.
  if (DUE_DIRECT_NUMERIC_RE.test(raw) || DUE_DIRECT_DAY_NAMED_RE.test(raw) ||
      DUE_DIRECT_NAMED_RE.test(raw)) return null;
  const numeric = raw.match(new RegExp(
    DUE_DIRECT_PREFIX + String.raw`(\d{1,2})[/.-](\d{1,2})(?![\d/.-])`, 'i'));
  if (numeric) {
    return isoDate(2000, Number(numeric[2]), Number(numeric[1])) ? Number(numeric[1]) : null;
  }
  const named = raw.matchAll(new RegExp(
    DUE_DIRECT_PREFIX + String.raw`(?:(\d{1,2})[-\s]+([A-Za-z]{3,9})|([A-Za-z]{3,9})\.?\s+(\d{1,2}))(?!\w|\s*[-/]?\s*\d)`, 'gi'));
  for (const match of named) {
    const month = MONTH_NAMES[(match[2] ?? match[3]).slice(0, 3).toLowerCase()];
    const day = Number(match[1] ?? match[4]);
    // "total due AED 3,240" is money, not a month called AED and day 3.
    if (month && isoDate(2000, month, day)) return day;
  }
  return null;
}
...
function parseSmsInner(
  ...
  const raw = sanitizeLocalMoney(foldOrthography(clean));
  ...
  // forecast its own evidence of settlement and nothing was ever suppressed.
  {
    ...
    if (settled !== raw && !SETTLED_MOVEMENT_RE.test(settled) && !settledFieldList) return null;
  }
  ...
  const NOT_A_BILLER_NAME =
    /\b(?:ACCOUNT|ACCT|A\/C|CARD|REF|REFERENCE|TXN|TRANSACTION|MOBILE|CUSTOMER|CONSUMER|INVOICE|BILL|POLICY|FILE|ORDER|TICKET|PLATE|TRN|IBAN|CHEQUE|CHECK)\b/i;
  ...
  if (billerRef && !NOT_A_BILLER_NAME.test(billerRef[1].trim())) {
    ...
    if (amountFils) {
      ...
      return {
        ...
        snapshotFils,
        snapshotKind,
        // A reference number establishes no kind of service. Unknown payees
        // stay Other, and a user override always retains its own category.
  ...
  // Flattened history can contain "Pymt due2022-06-06"; falling through there
  // promoted its minimum into spending. Purchase verbs retain their own path.
  if (
    (card || /\b(?:credit|covered)\s*card\b|\bcard\s+statement\b/i.test(raw)) &&
    STATEMENT_RE.test(raw) &&
    !STATEMENT_TXN_BLOCK_RE.test(raw) &&
    (MIN_DUE_RE.test(raw) || AR_MIN_DUE_RE.test(raw)) &&
    statementTotalFils(raw) === null
  ) return null;
  ...
  const isIncomingCreditReversal = INCOMING_CREDIT_REVERSAL_RE.test(prose);
  const isRefund = !isIncomingCreditReversal && (REFUND_RE.test(prose) || REVERSAL_RE.test(prose));
  ...
  const type: TransactionType = isBillDue || billerPaid || isIncomingCreditReversal
    ? 'expense'
  ...
  const ownedDestinationMove = OWN_DESTINATION_RE.test(prose) && OUTGOING_MOVE_RE.test(prose);
  let transferHint =
    !isBillDue &&
  ...
  } else if (explicitOwnAccountMove || ownedDestinationMove) {
  ...
  // events, not unidentified transfers. Keep this narrow recognition while
  // reverting the later masked-account routing/accounting experiment.
  if (!isBillDue && type === 'income') {
    const incomeText = raw.toLowerCase();
    if (/\bprofit\s+of\b(?:[^.\n]|\.\d){0,96}?\bcredited\s+to\s+your\s+(?:account|a\/?c)\b/i.test(raw)) {
      merchant = 'Bank profit';
      titlePinned = true;
    } else if (incomeText.includes('interest payout') ||
        (incomeText.includes('bonus multiplier') && incomeText.includes('interest'))) {
      merchant = 'Interest';
      titlePinned = true;
    } else if (incomeText.includes(' rev-') || incomeText.includes(' rev;') || incomeText.includes(' rev:')) {
      merchant = 'Refund';
      titlePinned = true;
    }
  }
...
export function parseSmsBatch(
  text: string,
  overrides?: Record<string, CategoryId>,
  interpret?: (message: string) => ParsedSms | null,
): ParsedSms[] {
  const rows = text
    .split(/\n\s*\n/)
    .map((m) => interpret ? interpret(m) : parseSms(m, overrides))
