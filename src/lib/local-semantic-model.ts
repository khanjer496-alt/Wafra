...
export interface LocalOnnxInt8TextEncoder {
  ...
  encode(text: string, options?: import('./local-semantic-scheduler').LocalSemanticEncodeOptions): Promise<ArrayLike<number>>;
}
...
export function createLocalSemanticRetriever(
  ...
  return Object.freeze({
    ...
    async retrieve(domain: LocalSemanticDomain, text: string, cancelled: () => boolean = () => false) {
      ...
      try {
        encoded = await encoder.encode(clean, { cancelled });
      } catch {
        return null;
      }
...
export async function chooseLocalSemanticAssistantPlan(input: {
  ...
  if (input.cancelled?.()) return deterministic('model-unavailable');
  ...
  const grounded = input.groundRequest ? input.groundRequest(compiled) : compiled;
  // The app-owned grounding callback may return planner clarification copy.
  // Model envelopes cannot carry this copy; executable plans still use the
  // strict public tool validator.
  if (grounded.tool !== 'help' && !isAssistantToolRequest(grounded)) return deterministic('compiled-request-rejected');
  if (input.cancelled?.()) return deterministic('model-unavailable');
  return Object.freeze({
    source: 'local-semantic',
    request: freezeCompiledAssistantRequest(grounded),
