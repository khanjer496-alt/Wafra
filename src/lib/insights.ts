...
export function buildInsights(
  ...
  if (prev && previous.expenseFils > 0 && current.expenseFils > 0) {
    // A day-one or payday-heavy sample is not a trend, and a projection is a
    // multiplication: on day 4 of a 31-day month the pace is scaled by 7.75, so
    // the rent alone — one AED 5,500 charge that will not recur — projects to
    // over AED 42,000 and the card announced "Trending 300% higher · you'll
    // spend AED 48,122 this month" against a month that ended nowhere near it.
    // Claims such as "1,511% higher" came out of the same arithmetic.
    //
    // So the insight waits until the multiplier is small enough for one lumpy
    // charge not to decide it: a full week at minimum, and at least half the
    // period, which caps the scaling at 2x. Before that there is nothing
    // honest to say about the month's pace, and the month-over-month card and
    // the daily average still have the ledger covered.
    if (live) {
      if (dayOfMonth >= 7 && dayOfMonth * 2 >= totalDaysInPeriod) {
  ...
  // `isFixedCommitment` rather than a hand-written rent/business pair: the same
  // list drives the weekday chart in analytics.ts, and two copies of it would
  ...
  if (current.incomeFils > 0) {
    ...
    if (rate >= 0.2) {
      ...
      insights.push({
        ...
        // Scoped to what the sentence is about. An unfiltered list opens on
        // rows that include the income the card just said was not enough.
        href: dest('/transactions', { type: 'expense' }),
      });
    }
  }
