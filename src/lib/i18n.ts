...
const S = {
  ...
  iosMessageGuideNoFilter: { en: 'In “Message Contains” type a single space (press the space bar once) so Next turns on. Every message then runs the shortcut; Wafra keeps bank alerts and discards other messages on this iPhone.', ar: 'في «تحتوي الرسالة على» اكتب مسافة واحدة فقط (اضغط مفتاح المسافة مرة واحدة) حتى يُفعَّل زر التالي. عندها تُشغِّل كل رسالة الاختصار؛ يحتفظ وفرة بتنبيهات البنك ويتجاهل بقية الرسائل على هذا الآيفون.' },
  ...
  iosMessageGuideRunShortcut: { en: 'Run {shortcut} · full Received Message', ar: 'شغّل {shortcut} · الرسالة المستلمة كاملة' },
