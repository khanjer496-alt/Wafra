...
export const homeSummaryCopy = {
  en: { balance: 'Recorded balances', net: 'Net after spending', notBalance: 'Income minus spending · not your bank balance',
    ...
    // Wafra sees only what reached the ledger, never the inbox: say "captured".
    stoppedIos: (days: number) => `No payments captured for ${days} ${days === 1 ? 'day' : 'days'}`,
    stoppedAndroid: (days: number) => `No payments captured for ${days} ${days === 1 ? 'day' : 'days'}`,
    rhythmSeveral: 'Usually several are captured a day.', rhythmDaily: 'Usually about one is captured a day.',
    rhythmEvery: (days: number) => `Usually one is captured every ${days} days.`,
  ...
  ar: { balance: 'الأرصدة المسجلة', net: 'الصافي بعد الإنفاق', notBalance: 'الدخل ناقص الإنفاق · ليس رصيد البنك',
    ...
    stoppedIos: (days: number) => `لم تُلتقط أي دفعة منذ ${arabicDays(days)}`,
    stoppedAndroid: (days: number) => `لم تُلتقط أي دفعة منذ ${arabicDays(days)}`,
    rhythmSeveral: 'عادةً تُلتقط عدة دفعات يومياً.', rhythmDaily: 'عادةً تُلتقط دفعة واحدة تقريباً كل يوم.',
    rhythmEvery: (days: number) => `عادةً تُلتقط دفعة كل ${arabicDays(days)}.`,
...
export const entryDetailCopy = {
  en: { statementPdf: 'Bank statement (PDF)', statementCsv: 'Bank statement (CSV)', applePay: 'Apple Pay (Wallet)', bankEmail: 'Bank email',
    ...
    markTransferBody: 'It stays in account balances but no longer counts as spending or income. You can change this later from the transaction.',
  ...
  ar: { statementPdf: 'كشف حساب بنكي (PDF)', statementCsv: 'كشف حساب بنكي (CSV)', applePay: 'Apple Pay (المحفظة)', bankEmail: 'بريد البنك',
    ...
    markTransferBody: 'يبقى في أرصدة الحسابات لكنه لا يُحتسب إنفاقاً أو دخلاً. يمكنك تغيير ذلك لاحقاً من المعاملة نفسها.',
