...
export interface ReviewAlert {
  ...
  market: Exclude<DetectedMarket, 'AE' | 'SA'>;
  institution: string;
...
export const normalizeAlertReviewTray = (value: unknown, now: number): AlertReviewTrayState => {
  ...
  const safePending = candidate.pending.filter((item): item is ReviewAlert => {
    ...
    ['synthetic-seed', 'consented-redacted', 'public-template'].includes(grammar.provenance) &&
  ...
  return pruneAlertReviewTray({ schemaVersion: 1, pending: safePending, tombstones: safeTombstones }, now);
};
