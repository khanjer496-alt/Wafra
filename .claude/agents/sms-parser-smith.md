...
Four suites, in this order: **parser, bank-corpus, invariants, unit**, then
**worker**. All of them must pass before you report anything as done.
`run.sh` sed-transpiles `src/lib/*.ts` into `scripts/test/build/` first, so a
change is only really tested after a fresh run — never `node` a stale
`build/` file and believe it.

**parser.test.js and bank-corpus.test.js are each blind to what the other
covers, and a green run of one proves nothing.** They came from two branches
that were merged: parser.test.js is the regression net for SUPPRESSION —
declines, OTP footers, pre-auth holds, offers, spend summaries, the whole
class of "this message is not a transaction" — and it stayed green while six
real credit cards were typed as debit and six real messages carried no date.
bank-corpus.test.js and invariants.test.js are the net for EXTRACTION — card
identity, dates, snapshots, FX provenance, the settlement side — and they
stayed green while a AED 10,000 salary was booked as an expense. Run both.

invariants.test.js also prints a coverage line ("N unresolved (X%)"). It is
reported, not asserted, so it will not fail a run — but a rule that pushes it
UP has almost certainly taken a category away from something.
